...
const ReportsPage = ({ user }) => {
  ...
  useEffect(() => {
    ...
    .filter(item => item && item.Location && item.Location.latitude && item.Location.longitude && item.ReportType)
    .map((item, index) => ({
  ...
  return (
    <div>
      {filteredData.map((item, index) => (/* something to note: when the width is about 420-999px, the tabs are wider than page*/
        <div key={index} style={{margin: '20px', border: '1px solid #ddd', padding: '10px', borderRadius: '5px'}}>
          ...
          <p><strong>Alert Type:</strong> {item.AlertType}</p>
          <p><strong>Report Type:</strong> {item.ReportType}</p>
          <p><strong>Description:</strong> {item.Description}</p>
          <p><strong>Address:</strong> {item.Address}</p>
          <p><strong>Time:</strong>  {item.Time ? item.Time.toDate().toISOString() : 'N/A'}</p>
          <p><strong>Author ID:</strong> {item.AuthorID}</p>
        </div>
      ))}
